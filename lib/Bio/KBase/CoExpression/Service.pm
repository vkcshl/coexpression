package Bio::KBase::CoExpression::Service;
...
our %return_counts = (
        'filter_genes' => 1,
        'const_coex_net_clust' => 1,
        'version' => 1,
);

our %method_authentication = (
        'filter_genes' => 'required',
        'const_coex_net_clust' => 'required',
);
...
sub call_method {
    ...
    my $ctx = Bio::KBase::CoExpression::ServiceContext->new($self->{loggers}->{userlog},
...
{
    # Service CoExpression requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for CoExpression but no authentication header was passed");
	}
	...
	my $stderr = Bio::KBase::CoExpression::ServiceStderrWrapper->new($ctx, $get_time);
	$ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service CoExpression.");
    }
...
package Bio::KBase::CoExpression::ServiceContext;

use strict;

=head1 NAME

Bio::KBase::CoExpression::ServiceContext
...
package Bio::KBase::CoExpression::ServiceStderrWrapper;
